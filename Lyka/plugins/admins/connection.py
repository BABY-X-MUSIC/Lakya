...
from Lyka.utils.database import connect_to_chat
from Lyka.utils.decorators import AdminActual
...
async def auth(client, message: Message):
   ...
   # re = await connect_to_chat(message.from_user.id, message.chat.id)
    ...
    markup = InlineKeyboardMarkup([[InlineKeyboardButton("Connect to chat", url=f"http://t.me/{app.username}?start=connect_{chat_id}")]])
    await message.reply_text("Tap the following button to connect to this chat in PM", reply_markup = markup)
