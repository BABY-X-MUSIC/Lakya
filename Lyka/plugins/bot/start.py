    ...
    if len(message.text.split()) > 1:
        ...
        if name[:8] == "connect_":
            ...
            try:
                ...
                return await message.reply_text(f"Look like i am not an admin of the chat group {chat_id}")
            ...
            if message.from_user.id not in admin_ids:
                return await message.reply_text(f"Sorry sir but i think that you are not an admin of {title} ")
            ...
            if member.id == app.id:
                ...
                if message.chat.id in await blacklisted_chats():
                    await message.reply_text(
                        _["start_5"].format(
                            app.mention,
                            f"https://t.me/LykaRobot?start=sudolist",
                            config.SUPPORT_CHAT,
                        ),
