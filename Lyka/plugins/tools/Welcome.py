...
def welcomepic(pic, user, chatname, id, uname):
    background = Image.open("Lyka/assets/Kr.png")
    pfp = Image.open(pic).convert("RGBA")
    pfp = circle(pfp)
    pfp = pfp.resize((1157, 1158))
    draw = ImageDraw.Draw(background)
    font = ImageFont.truetype('Lyka/assets/font.ttf', size=110)
    welcome_font = ImageFont.truetype('Lyka/assets/font.ttf', size=60)
...
async def auto_state(_, message):
    ...
    if user.status in (
        ...
        if state == "off":
            ...
            if not A:
                await message.reply_text("**Enable welcome notification.**")
            else:
        ...
        await message.reply("**Sorry only admins can enable welcome notification!**")
