...
async def search_(app: app, msg: Message):
    ...
    if len(split) == 1:
        return await msg.reply_text("**Give Query To Search**")
    to_del = await msg.reply_text("**Searching On Google...**")
    query = split[1]
    try:
        ...
        txt = f"**Here are the results of requested: {query.title()}**"
        await to_del.delete()
        await msg.reply_text(txt, reply_markup=keyboard)
        return
    except NoResultsFound:
        await to_del.delete()
        await msg.reply_text("**No result found corresponding to your query**")
        return
    except NoResultsOrTrafficError:
        await to_del.delete()
        await msg.reply_text("****No result found due to too many traffic**")
        return
    except Exception as e:
        await to_del.delete()
        await msg.reply_text(f"**Something went wrong :\nReport at it** @Nitriic")
        print(f"error : {e}")
        return
